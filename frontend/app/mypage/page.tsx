...
export default function MyPage() {
  ...
  const { subscription, isSubLoading, refetchSubscription } =
  ...
  }, [router, fetchSavedCourses, fetchBillingCards, fetchPaymentHistories])
  ...
  async function handleRegisterCard() {
    ...
    if (!window.TossPayments) {
      toast.error(
        "토스페이먼츠 라이브러리가 아직 로드되지 않았습니다. 잠시 후 다시 시도해주세요.",
      )
      return
    }
                ...
                {isBillingLoading ? (
                  ...
                  <div className="grid gap-5 sm:grid-cols-2">
                    {billingCards.map((card) => (
                      <Card
                        key={card.id}
                        className="relative flex min-h-[160px] flex-col justify-between overflow-hidden border-0 bg-gradient-to-br from-slate-900 to-slate-800 p-6 text-white shadow-sm"
                      >
                        <div>
                          ...
                          <p className="mt-4 font-mono text-lg tracking-widest text-slate-100">
                            {card.cardNumber}
                          </p>
                        </div>
                        ...
                        <div className="mt-4 flex items-end justify-between">
                          ...
                          <Button
                            ...
                            size="sm"
                            variant="destructive"
                            className="h-8 gap-1 border-0 bg-red-600/90 text-white hover:bg-red-600"
                    ...
                    <button
                      ...
                      className="group flex min-h-[160px] flex-col items-center justify-center gap-2.5 rounded-xl border-2 border-dashed border-border/80 bg-transparent p-6 text-muted-foreground transition-all hover:border-primary/50 hover:text-primary disabled:opacity-50"
                    >
            ...
            <Card className="border-border/60">
              ...
              <CardContent>
                {subscription && subscription.status !== "CANCELED" ? (
                  <div className="flex flex-col gap-6 rounded-2xl border border-border/80 bg-secondary/10 p-6 sm:flex-row sm:items-center sm:justify-between">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        {subscription.status === "CANCELED_RESERVED" ? (
                          <Badge className="border-0 bg-yellow-500/10 text-yellow-600 hover:bg-yellow-500/10">
                            해지 예약됨
                          </Badge>
                        ) : (
                          <Badge className="border-0 bg-green-500/10 text-green-600 hover:bg-green-500/10">
                            이용 중
                          </Badge>
                        )}
                      </div>
                    ...
                    {subscription.status === "ACTIVE" && (
                      <Button
                        type="button"
                        variant="outline"
                        className="h-9 shrink-0 gap-1.5 border-destructive/30 text-destructive hover:bg-destructive/5 hover:text-destructive"
                  ...
                  <div className="py-10 text-center">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      구독 멤버십에 가입하시면 최적화된 고퀄리티 여행 루트 추천
                      패키지를 이용하실 수 있어요.
                    </p>
                    <Button asChild className="mt-5 text-xs">
