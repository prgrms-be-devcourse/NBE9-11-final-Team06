...
public interface BillingInfoRepository extends JpaRepository<BillingInfo, Long> {
    ...
    List<BillingInfo> findByMemberIdAndStatusOrderByCreatedAtDesc(Long memberId, BillingStatus status);

    Optional<BillingInfo> findByBillingKeyAndStatus(String billingKey, BillingStatus status);
    ...
    Optional<BillingInfo> findByIdAndMemberIdAndStatus(Long id, Long memberId, BillingStatus status);
}
